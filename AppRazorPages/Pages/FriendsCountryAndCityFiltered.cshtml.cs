...
{
    public class FriendsCountryAndCityFilteredModel : PageModel
    {
        private readonly IHttpClientFactory _httpClientFactory;

        public FriendsCountryAndCityFilteredModel(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task OnPostCallFriendsCountryAndCityFilteredAsync()
        {
            try
            {
        ...
        {
            await OnPostCallFriendsCountryAndCityFilteredAsync();
            CountryList = FriendsByCountry.Select(g => g.Key).ToList();
            CityList = FriendsData.PageItems
                .Where(a => a.City != null && !string.IsNullOrEmpty(a.City))
                .Select(a => a.City)
                .Distinct()
                .ToList();
        }
